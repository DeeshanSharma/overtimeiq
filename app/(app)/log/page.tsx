  ...
  function handlePunchOut() {
    ...
    exec(
      `INSERT INTO logs (job_id, date, start_time, end_time, crosses_midnight, duration_hours,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'draft', 'punch', ?, ?)`,
      [activeSnap.job_id, date, startTime, endTime, result.crossesMidnight ? 1 : 0,
       result.duration, activeSnap.location, activeSnap.project || null, nowIso, nowIso]
    );
    loadAll();
  }
    ...
    <div style={{ maxWidth: "600px", margin: "0 auto", padding: "0 0 80px" }}>
      ...
      {activeSession ? (
        <div style={{ margin: "16px 20px", padding: "20px", border: "1px solid #16a34a", background: "#f0fdf4" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <p style={{ fontSize: "0.68rem", letterSpacing: "0.1em", color: "#16a34a", textTransform: "uppercase", margin: "0 0 4px" }}>Live session</p>
              <p style={{ fontFamily: "var(--font-serif)", fontSize: "2rem", color: "#0e0e0e", margin: "0 0 4px", letterSpacing: "-0.02em" }}>
                {fmtElapsed(elapsed)}
              </p>
            ...
            <button
              ...
              style={{ padding: "12px 20px", background: "#dc2626", color: "white", border: "none", fontFamily: "var(--font-mono)", fontSize: "0.82rem", fontWeight: 500, cursor: "pointer" }}
            >
              Punch out
            </button>
          </div>
      ...
      {logs.length === 0 ? (
        <div style={{ textAlign: "center", padding: "64px 20px", color: "#6b6b5e" }}>
          <p style={{ fontFamily: "var(--font-serif)", fontSize: "1.3rem", color: "#0e0e0e", marginBottom: "8px" }}>No entries yet.</p>
      ...
      {(modal === "edit" && editLog) && (
        <ManualEntryModal onClose={() => { setModal(null); setEditLog(null); }} editLog={editLog} />
      )}
...
function LogRow({
  ...
  const statusColor: Record<string, string> = {
    draft: "#6b6b5e", submitted: "#d97706", approved: "#16a34a",
  };
    ...
    <div style={{ borderBottom: "1px solid #d1c9b8" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: log.job_color ?? "#3B8BD4", flexShrink: 0 }} />
          <div>
        ...
        <div style={{ textAlign: "right" }}>
          <p style={{ fontSize: "0.72rem", color: statusColor[log.status] ?? "#6b6b5e", margin: "0 0 2px", textTransform: "uppercase", letterSpacing: "0.06em" }}>
            {log.status}
          </p>
      ...
      {open && (
        <div style={{ padding: "0 20px 14px 40px", display: "flex", gap: "10px", flexWrap: "wrap" }}>
          ...
          {log.notes ? (
            <span style={{ fontSize: "0.72rem", color: "#6b6b5e", flex: 1 }}>{log.notes}</span>
          ) : null}
          <div style={{ display: "flex", gap: "8px", marginLeft: "auto" }}>
            <button onClick={e => { e.stopPropagation(); onEdit(); }} style={actionBtn}>Edit</button>
            <button onClick={e => { e.stopPropagation(); onDelete(); }} style={{ ...actionBtn, color: "#dc2626", borderColor: "#dc2626" }}>Delete</button>
          </div>
        </div>
      )}
    </div>
...
const actionBtn: React.CSSProperties = {
  padding: "4px 12px", fontSize: "0.72rem", fontFamily: "var(--font-mono)",
  border: "1px solid #d1c9b8", background: "none", cursor: "pointer", color: "#6b6b5e",
};
